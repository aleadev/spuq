...
def test4():
    ...
    for j, ex in enumerate(EX):
        ...
        for degree in range(1, 5):
            ...
            F3.interpolate(ex)
            F4.interpolate(ex)
            err1, _ = mv.get_projection_error_function(mis[1], mis[0], degree, refine_mesh=0)
            err2, _ = mv.get_projection_error_function(mis[2], mis[0], degree, refine_mesh=0)
            err3, _ = mv.get_projection_error_function(mis[3], mis[0], degree, refine_mesh=0)
            ...
            err1, _ = mv.get_projection_error_function(mis[1], mis[0], degree, refine_mesh=2)
            err2, _ = mv.get_projection_error_function(mis[2], mis[0], degree, refine_mesh=2)
            err3, _ = mv.get_projection_error_function(mis[3], mis[0], degree, refine_mesh=2)
