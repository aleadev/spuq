...
def evaluate_Hermite_triple(I_a, I_b, I_c):
    ...
    for i in range(m):
        ind = np.transpose(np.tile(I_a[:,i], [1, nb, nc]), [0, 1, 2])
        ind = ind + strides[1] * np.transpose(np.tile(I_b[:,i], [1, na, nc]), [1, 0, 2])
        ind = ind + strides[2] * np.transpose(np.tile(I_c[:,i], [1, na, nb]), [1, 2, 0])
        Mi = triples[ind]
        if i == 0:
            M = Mi
        else:
...
def compute_Hermite_triples(p):
    ...
    I, J, K = np.mgrid[1:p+1, 1:p+1, 1:p+1]
    S = I + J + K
    S2 = S/2
