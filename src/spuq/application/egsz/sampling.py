...
def compute_direct_sample_solution(pde, RV_samples, coeff_field, A, maxm, proj_basis, Dirichlet_boundary=lambda x, on_boundary: on_boundary):
    try:
        A0 = coeff_field.A
        A_m = coeff_field.A_m
...
def compute_direct_sample_solution_old(pde, RV_samples, coeff_field, A, maxm, proj_basis, Dirichlet_boundary=lambda x, on_boundary: on_boundary):
    a = coeff_field.mean_func
    for m in range(maxm):
    ...
    b = pde.assemble_rhs(proj_basis, Dirichlet_boundary=Dirichlet_boundary)
    X = 0 * b
    solve(A, X, b)
