...
class EquilibrationEstimator(object):
    ...
    def _evaluateGlobalMixedEstimator(cls, mu, w, coeff_field, pde, f, quadrature_degree):
        ...
        for m in range(maxm):
            ...
            sigma_mu = sigma_mu + am_f * nabla_grad(r_mu)
        ...
        mesh = V.mesh()
        degree = pde.element_degree(w)

        # create function spaces
        DG = FunctionSpace(mesh, 'DG', 0)
        RT = FunctionSpace(mesh, 'RT', degree-1)
        W = RT*DG
        ...
        L_eq = -f_mu*v*dx + sigma_mu*tau*dx

        # compute solution
        w_eq = Function(W)
        solve(a_eq == L_eq, w_eq, bc)
